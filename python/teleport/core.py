...
from teleport import libteleport
...
J = libteleport.teleport_JsonValue
...
def wrap_json(pythonic_json_value):
    ...
    if c in (int, float):
        ...
        d = libteleport.haxe_ds_StringMap()
...
def deserialize_json_object(string_map):
    return string_map.h


class Teleport(libteleport.teleport_Teleport):
    def __deserialize(self, schema, json_value):
        return super(Teleport, self).deserialize(schema, json_value)

    def deserializeJsonObject(self, string_map):
        raise RuntimeError()
        # return string_map.h
...
class Type(object):
    def __init__(self, json_value):
        ...
        e = libteleport.teleport_SchemaTools.schemaFromJsonValue(self.j)
        if _is(e, 'Right'):
            self.schema = _get(e)
        else:
