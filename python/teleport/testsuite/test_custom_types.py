...
class CustomTypeTest(unittest2.TestCase):
    ...
    def test_Color(self):
        self.assertTrue(t("Color").check('#ffffff'))
        self.assertFalse(t("Color").check('yellow'))
        self.assertTrue(t({"Array": "Color"}).check(['#ffffff', '#000000']))
    ...
    def test_Nullable(self):
        ...
        self.assertTrue(s.check(["sparse", None, "arrays", None, None, None, "what"]))
        ...
        self.assertTrue(s.check({"id": 1, "name": "Jake", "age": 28}))
        self.assertTrue(s.check({"id": 1, "name": None, "age": 12}))
        self.assertTrue(s.check({"id": 1, "age": None}))
